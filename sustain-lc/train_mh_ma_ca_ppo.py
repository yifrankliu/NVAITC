...
def train():
    ...
    print_freq            = max_ep_len * 5   # print avg reward in the interval (in num timesteps)10
    log_freq              = max_ep_len * 5    # log avg reward in the interval (in num timesteps)5
    ...
    reward_fn_version  = 2
    run_num_pretrained = 2  # reward_fn_version  # change this to prevent overwriting weights in same env_name folder
    exogen_gen_v       = 2   # version of exogensous variable iterator to use; possible choices for v are 1 and 2
    
    env = MH_SmallFrontierModel(use_reward_shaping = f'reward_shaping_v{reward_fn_version}', exogen_gen_v = exogen_gen_v)
    ...
    log_f_name = log_dir + '/MH_MA_CA_PPO_' + env_name + "_log_" + str(run_num) + ".csv"
