...
model LimPID_Hysteresis "PID controller with hysteresis"
  import InitPID =
...
//   parameter Real k_m= 1 "Measurement input scaling: k_m*u_m. May improve simulation";
  ...
  parameter Real hysteresisOn = 1
    "if off and control error > hysteresisOn, switch to set point tracking"
    annotation (Dialog(group="Hysteresis"));
  parameter Real hysteresisOff = -hysteresisOn "if on and control error < hysteresisOff, set y=0"
    annotation (Dialog(group="Hysteresis"));
  ...
  Modelica.Blocks.Logical.Hysteresis Hysteresis(uLow=hysteresisOff, uHigh=
        hysteresisOn) "Turn controller On/Off"
...
equation
  assert(hysteresisOff < hysteresisOn, "Wrong controller parameters. Require hysteresisOff < hysteresisOn.");
  ...
  connect(Hysteresis.y, swi1.u2)
    annotation (Line(points={{-1,60},{38,60}}, color={255,0,255}));
  connect(Hysteresis.u, feeBac.y)
    annotation (Line(points={{-24,60},{-51,60}}, color={0,0,127}));
  connect(feeBac.y, swi1.u3) annotation (Line(points={{-51,60},{-40,60},{-40,30},
          {32,30},{32,52},{38,52}}, color={0,0,127}));
