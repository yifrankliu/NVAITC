...
model EHX_Staging "EHX staging logic"
  ...
  // max. no of EHXs
  parameter Real nCT_crit[nEHX_max] = {1, 4, 8, 12} "no of cooling towers for staging EHXs" annotation(Dialog(group="Inputs"));  // Criteria for staging EHXs
  parameter Real valve_opening_min = 0.01 "Minimium opening of valves";
  Real valve_int[nEHX_max];
...
initial equation
  ...
  valveEHXs[nEHX_start+1:nEHX_max] = valve_opening_min*ones(nEHX_max-nEHX_start);
equation
  ...
  when {max(nCT, 7) >= nCT_crit[min(pre(nEHX) + 1, nEHX_max)], max(nCT, 7) < nCT_crit[pre(nEHX)]} then
    if max(nCT, 7) >= nCT_crit[min(pre(nEHX) + 1, nEHX_max)] then
      nEHX = min(max(pre(nEHX) + 1, nEHX_min), nEHX_max);
    elseif max(nCT, 7) < nCT_crit[pre(nEHX)] then
      nEHX = min(max(pre(nEHX) - 1, nEHX_min), nEHX_max);
    else
    ...
    for idx in 1:nEHX_max loop
      startTime[idx] = time;
      if max(nCT, 7) >= nCT_crit[idx] then
  ...
  for idx in 1:nEHX_max loop
    ...
    valveEHXs[idx] = Modelica.Media.Air.MoistAir.Utilities.smoothMax(valve_int[idx],valve_opening_min, 0.5*valve_opening_min);
  end for;
