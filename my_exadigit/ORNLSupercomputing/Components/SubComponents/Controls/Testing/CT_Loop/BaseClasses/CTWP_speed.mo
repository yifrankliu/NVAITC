...
model CTWP_speed
  ...
  parameter Real CTWP_Nrel_min = 43.0 "min. CTWP Nrel %" annotation(Dialog(group="General Inputs"));
  parameter Real CTWP_Nrel_max = 70.0 "max. CTWP Nrel %" annotation(Dialog(group="General Inputs"));
  parameter Real CTWP_Nrel_start = 45.0 "starting CTWP Nrel %" annotation(Dialog(group="General Inputs"));
  ...
  parameter Real wp = 0.05 "PID setpoint weighting" annotation(Dialog(group="PID Inputs"));
...
//   Real delayed_p_setpoint;
  ...
  BaseClasses.p_CTWR_setpoint p_CTWR_Setpoint_Model
  ...
  Modelica.Blocks.Interfaces.RealOutput p_CTWR_setpoint "CTWP Nrel" annotation (
     Placement(transformation(
        ...
        rotation=180,
        origin={-40.4,26})));
  Modelica.Blocks.Continuous.Filter filter(filterType=Modelica.Blocks.Types.FilterType.LowPass, f_cut=
        f_cut)
    annotation (Placement(transformation(extent={{-4,25},{10,11}})));
...
equation
  ...
  connect(p_CTWR_Setpoint_Model.p_CTWR_setpoint, PID_CTWP.u_s) annotation (Line(
        points={{-25.96,2.4},{-10,2.4},{-10,-11},{2.2,-11}},  color={0,0,127}));
  connect(filter.y, PID_CTWP.u_m)
    annotation (Line(points={{10.7,18},{13,18},{13,-0.2}},
                                                        color={0,0,127}));
  connect(p_CTWR_Setpoint_Model.p_CTWR_setpoint, filter.u) annotation (Line(
        points={{-25.96,2.4},{-10,2.4},{-10,18},{-5.4,18}},    color={0,0,127}));
end CTWP_speed;
