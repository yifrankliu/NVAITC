...
model Blade_simple_volume
  ...
  parameter Modelica.Units.SI.Volume ccVolBlade = 0.3;
  parameter TRANSFORM.Units.HydraulicResistance R_Blade = 11000;
  ...
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance dP_Blade_in(
      redeclare package Medium = Medium, R=R_Blade*0.5)
    annotation (Placement(transformation(extent={{-70,-10},{-50,10}})));
  TRANSFORM.Fluid.FittingsAndResistances.SpecifiedResistance dP_Blade_out(
      redeclare package Medium = Medium, R=R_Blade*0.5)
...
equation
  ...
  connect(port_a, dP_Blade_in.port_a)
    annotation (Line(points={{-100,0},{-67,0}}, color={0,127,255}));
  connect(dP_Blade_in.port_b, coolingChannels.port_a)
    annotation (Line(points={{-53,0},{-6,0}}, color={0,127,255}));
  connect(coolingChannels.port_b, dP_Blade_out.port_a)
    annotation (Line(points={{6,0},{47,0}}, color={0,127,255}));
  connect(dP_Blade_out.port_b, port_b)
    annotation (Line(points={{61,0},{100,0}}, color={0,127,255}));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{
            100,100}})),
    experiment(StopTime=22000, __Dymola_Algorithm="Esdirk45a"));
end Blade_simple_volume;
