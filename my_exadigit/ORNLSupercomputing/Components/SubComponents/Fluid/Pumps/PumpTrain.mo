...
model PumpTrain
  ...
  TRANSFORM.Fluid.Valves.ValveLinear valve(
    dp_start=500,
    dp_nominal=1000,
    m_flow_nominal=port_a_nominal.m_flow) constrainedby
    TRANSFORM.Fluid.Valves.ValveLinear(redeclare package Medium = Medium)
  ...
  TRANSFORM.Fluid.Machines.Pump_Controlled pump(
    ...
    m_flow_nominal=port_a_start.m_flow) constrainedby
    TRANSFORM.Fluid.Machines.Pump_Controlled(redeclare package Medium = Medium,
      use_port=true)     annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={40,0})), choicesAllMatching=true);
