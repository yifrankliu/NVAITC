...
partial model PartialFourPort_across
  ...
  replaceable TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_a1
    ...
    h_outflow(start=Medium_1.h_default)) annotation (Placement(
        transformation(extent={{-190,50},{-170,70}}), iconTransformation(
          extent={{-110,50},{-90,70}})));
  ...
  replaceable TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_a2
    ...
    m_flow(min=if allowFlowReversal then -Modelica.Constants.inf else 0),
    h_outflow(start=Medium_2.h_default)) annotation (Placement(
        transformation(extent={{170,-70},{190,-50}}), iconTransformation(
          extent={{90,-70},{110,-50}})));
  replaceable TRANSFORM.Fluid.Interfaces.FluidPort_Flow port_b2
    ...
    m_flow(max=if allowFlowReversal then +Modelica.Constants.inf else 0),
    h_outflow(start=Medium_2.h_default)) annotation (Placement(
        transformation(extent={{-190,-70},{-170,-50}}), iconTransformation(
          extent={{-110,-70},{-90,-50}})));
